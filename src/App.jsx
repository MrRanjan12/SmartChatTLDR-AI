...
export default function App() {
  ...
  return (
  <div className={`flex flex-col h-screen w-screen ${isDark ? "bg-[#1f1f1f] text-white" : "bg-white text-black"}`}>
    {/* Header */}
    <header
    ...
    >
      <div className="flex items-center gap-4">
        <button className="sm:hidden" onClick={toggleSidebar}><Menu size={24} /></button>
    ...
    {/* Sidebar */}
    <aside
      className={`fixed top-16 left-0 h-[calc(100vh-4rem)] w-64 z-30 transition-transform duration-300 transform ${
    ...
    >
      ...
      <nav className="space-y-2">
        <button className="w-full text-left py-2 px-3 rounded hover:bg-cyan-100 transition">Futuristic AI Chat UI</button>
        <button className="w-full text-left py-2 px-3 rounded hover:bg-cyan-100 transition">Chatbot Integration Tutorial</button>
        <button className="w-full text-left py-2 px-3 rounded hover:bg-cyan-100 transition">Prototype App ! But U can use it.</button>
      </nav>
      <div className="mt-auto pt-6 border-t text-sm flex items-center gap-2">
        ...
        Account facility not yet
      </div>
    </aside>

    {/* Main Content Area */}
    <main className={`flex-1 pt-16 ${sidebarOpen ? "" : "sm:ml-64"} overflow-hidden`}>
      <div className="h-full flex flex-col">
        ...
        {/* Footer Input + Attribution */}
        <div
          className={`border-t ${isDark ? "bg-[#2a2a2a] border-gray-700" : "bg-gray-200 border-gray-300"}`}
        >
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div
              className={`text-center pb-3 text-sm ${
